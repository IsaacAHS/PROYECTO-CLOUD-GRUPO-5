...
HEADNODE_IP="${NIMBUSCORE_HEADNODE_IP:-10.0.10.5}"
IFS=',' read -r -a COMPUTE_IPS <<< "${NIMBUSCORE_COMPUTE_IPS:-10.0.10.1,10.0.10.2,10.0.10.3,10.0.10.4}"
...
if [ "$N_LINKS" -gt 0 ]; then
    ...
    echo ""
    echo "[PASO 1] Sin enlaces internos: las VMs tendran solo la NIC de gestion si esta habilitada."
fi
